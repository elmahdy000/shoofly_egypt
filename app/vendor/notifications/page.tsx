...
export default function VendorNotificationsPage() {
  ...
  return (
    <div className="space-y-4">
      <SectionHeader title="Notifications" subtitle="Vendor account alerts (auto-refresh every 4s)" />
      ...
      <div className="space-y-3">
        {(data ?? []).map((item) => (
          <button key={item.id} onClick={() => markRead(item.id)} className="w-full rounded-2xl border border-slate-200 bg-white p-4 text-left">
