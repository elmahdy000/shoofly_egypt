...
export async function settleOrder(requestId: number) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const configuredCommission = toTwo((netPrice * Number(platform.commissionPercent)) / 100);
    const commission = toTwo(clientPrice - netPrice);
    const finalCommission = commission > 0 ? commission : configuredCommission;
    const vendorPayout = toTwo(clientPrice - finalCommission);
